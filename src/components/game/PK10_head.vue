        ...
        <Row justify="start" align="middle">
            <Col span="12">
                <div @click="showProjectList">
                    <p class="lotteryNum">第<span class="prewNum">{{PK10_issue.current_num}}</span>期<Icon type="md-arrow-dropdown" /></p>
                    <p v-if="PK10_issue.PK10Result[0]===''">正在开奖中</p>
                    <ul v-else>
                        <li v-for="(item,index) in PK10_issue.PK10Result" :key="index"><span class="dice" >{{item}}</span></li>
                    </ul>
                </div>
            </Col>
            <Col span="12">
                <div class="period-down-time">
                    <p class="nexNum">距{{PK10_issue.next_num}}期截止</p>
...
export default {
    ...
    methods:{
        ...
        async getIssueListForRefresh(){
            ...
            let data = res.data;
            if(!data && !data.issueHistory && data.issueHistory.issues[0]){
                return
            }
        ...
        //2.title组件初始化，选择彩种，玩法都会调用，格式化当前这期的数据（PK10格式化）
        initformatDataIssue(data){
            ...
            if(wn_number){
                //获取开奖数据
                this.PK10_issue.PK10Result=wn_number.split(" ");
            }else{//4.否则开奖数据为空且重新请求
                this.PK10_issue.PK10Result = ['','','','','','','','','',''];
                //请求调用新的时间,和获奖结果
        ...
        //2.每一次倒计时初始化，选择彩种，玩法都会调用，格式化当前这期的数据（PK10格式化）
