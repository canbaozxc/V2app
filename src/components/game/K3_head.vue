        ...
        <Row justify="start" align="middle">
            <Col span="12">
                <div @click="showProjectList">
                    <p class="lotteryNum">第<span class="prewNum">{{K3_issue.current_num}}</span>期<Icon type="md-arrow-dropdown" /></p>
                    <p v-if="K3_issue.K3Result[0]===''">正在开奖中</p>
                    <span v-else class="dice" :class="item==''?'diceGif':'rdice'+item" v-for="(item,index) in K3_issue.K3Result" :key="index"></span>
                </div>
            </Col>
            <Col span="12">
                <div class="period-down-time">
                    <p class="nexNum">距{{K3_issue.next_num}}期截止</p>
...
export default {
    ...
    methods:{
        ...
        async getIssueListForRefresh(){
            ...
            let data = res.data;
            if(!data && !data.issueHistory && data.issueHistory.issues[0] && data.issueHistory.issues[0].wn_number){
                return
            }
            let wn_number = data.issueHistory.issues[0].wn_number;
            if(wn_number){//请求到了开奖结果
                this.$parent.newProjects();//获取开奖记录
                this.$parent.getDateTime();//获取投注记录
                this.connectCount = 0;
        ...
        //2.title组件初始化，选择彩种，玩法都会调用，格式化当前这期的数据（K3格式化）
        initformatDataIssue(data){
            ...
            if(wn_number){
                //获取开奖数据
                this.K3_issue.K3Result=wn_number.split("");
            }else{//4.否则开奖数据为空且重新请求
                this.K3_issue.K3Result = ['','',''];
                //请求调用新的时间,和获奖结果
        ...
        //2.每一次倒计时初始化，选择彩种，玩法都会调用，格式化当前这期的数据（K3格式化）
...
//头部
    .lottery-info{
        ...
        .dice{
            ...
            width: 0.26rem;
            height: 0.26rem;
            background-image: url("../../assets/game/diceK3.png");
        ...
        .diceGif{
            background-size: 100%;
            background-image: url("../../assets/game/diceK3.gif");
        }
        .period-down-time{
            padding-top: 0.1rem;
            font-size: 0.14rem;
