...
export default {
    name:'SSCway',
    components:{
        SSCnoteList:() => import("@/components/game/SSC/noteList.vue"),
        SSChunhe:() => import("@/components/game/SSC/common_hunhe.vue"),
        SSCcommon:() => import("@/components/game/SSC/common.vue"),
        SSCquick:() => import("@/components/game/SSC/common_quick.vue"),
        SSCchi:() => import("@/components/game/SSC/common_chi.vue"),
        SSCqiu:() => import("@/components/game/SSC/common_qiu.vue"),
        Notice,
    },
    ...
    methods:{
        init(data){//初始化所有SSC玩法数据，模板映射
        //console.log(SSCDataGameBall);
        if(SSCDataGameBall[data.series_way_id]){//防止数据延迟，只有数据真正传过来了才能执行
            let dataTpl = {};
            switch (data.series_way_id) {
                default:
                        console.log(SSCDataGameBall[data.series_way_id]);
                    
                        dataTpl = SSCDataGameBall[data.series_way_id];
