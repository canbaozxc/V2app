...
<Scroll :on-reach-bottom="handleReachBottom" :height="667">
    <div class="my-table">
        <ul class="tit"  :style="{width:tableWidth+'px'}">
            <li class="th"  :style="{width:item.width+'px'}" v-for="(item,index) in columnsItem" :key="index">{{item.title}}</li>
        </ul>
        <div class="bd" >
            <ul v-for="(col,i) in data" :key="i" :style="{width:tableWidth+'px'}">
                <li class="td" v-for="(item,index) in columnsItem" :key="index" :style="{width:item.width+'px'}" v-if="!item.render" >{{col[item.key]}}</li>
